...
def save_trade_log():
    try:
        conn = get_db_connection()
        trades_df = pd.read_sql_query("SELECT * FROM trades WHERE user_id = ?", conn, params=(current_user.id,))
        index = int(request.form.get('index')) - 1  # Adjust index to match DataFrame
...
def add_trade():
    try:
        ...
        def parse_and_format_date(date_str):
            date_formats = ["%Y-%m-%d", "%d-%m-%Y %H:%M:%S", "%d-%m-%Y", "%Y-%m-%d %H:%M:%S"]
            for fmt in date_formats:
                try:
                    parsed_date = datetime.strptime(date_str, fmt)
        ...
        new_trade = {
            ...
            'Closed': parse_and_format_date(request.form.get('Closed', '')),
            'Opening_Ref': request.form.get('Opening_Ref', ''),
            'Opened': parse_and_format_date(request.form.get('Opened', '')),
...
def add_open_trade():
    if request.method == 'POST':
        try:
            ...
            opened_date = parse_date(request.form.get('Opened', ''))
            if opened_date:
...
def close_trade():
    if request.method == 'POST':
        try:
            ...
            closed_date = parse_date(request.form.get('closed-date', ''))
            ...
            if closed_date:
                ...
                if closed_time:
                    closed_date_str += f' {closed_time}'
            else:
                closed_date_str = None  # Use None for better handling later
